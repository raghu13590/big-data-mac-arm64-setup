...
create_superset_config() {
    local secret_key=$1
    local config_dir="$SCRIPT_DIR/../configs/superset"
...
restart_services() {
    restart_service "superset_db" "$SCRIPT_DIR/../apps/superset/docker-compose-superset.yml" "superset_db"
    restart_service "superset_cache" "$SCRIPT_DIR/../apps/superset/docker-compose-superset.yml" "superset_cache"
    restart_service "superset" "$SCRIPT_DIR/../apps/superset/docker-compose-superset.yml" "superset"
}
...
initialize_superset() {
    ...
    if ! docker exec -i superset superset fab list-users | grep -q admin; then

...
test_hive() {
    ...
    docker exec -i hiveserver2 beeline -u jdbc:hive2://hiveserver2:10000 -n hive -p hive -e "CREATE DATABASE IF NOT EXISTS test_db;"

    # Create a test table and insert sample data
    docker exec -i hiveserver2 beeline -u jdbc:hive2://hiveserver2:10000 -n hive -p hive -e "USE test_db; CREATE TABLE IF NOT EXISTS test_table (id INT, name STRING); INSERT INTO test_table VALUES (1, 'John'), (2, 'Jane');"

    # Query the test table and verify the output
    output=$(docker exec -i hiveserver2 beeline -u jdbc:hive2://hiveserver2:10000 -n hive -p hive -e "SELECT * FROM test_db.test_table;")
    ...
    docker exec -i hiveserver2 beeline -u jdbc:hive2://hiveserver2:10000 -n hive -p hive -e "DROP TABLE IF EXISTS test_db.test_table; DROP DATABASE IF EXISTS test_db;"
}
